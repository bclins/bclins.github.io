...
Today we talked about how to find the max or min of a function with two variables.  Since there are two partial derivatives, you have to set them both equal to zero to find the critical points.


1. $f(x,y) = x^2 + xy + y^2 - 3x$ ([video](https://youtu.be/bMlA7WY2LvA))


![Types of Critical Points](https://www.offconvex.org/assets/saddle/minmaxsaddle.png)

<div class="Theorem">
**Second Derivative Test for Two Variable Functions**

Find the **determinant** $D = f_{xx} \cdot f_{yy} - (f_{xy})^2$.

* If $D < 0$, the critical point is a saddle point. 
* If $D > 0$, the critical point is either a local max or min (and you can tell by looking at the concavity $f_{xx}$).
