...
%%% NEXT TIME MAYBE INCLUDE THIS PROBLEM IF YOU DON'T PROVE THIS IN CLASS? NO, THIS MIGHT BE TOO EASY. 
%\item If $p(z) = a_n z^n + a_{n-1} z^{n-1} + \ldots + a_1 z + a_0$ is a polynomial with complex coefficients and degree $n > 0$, then prove that 
%$$\lim_{z \rightarrow \infty} \frac{p(z)}{z^n} = a_n.$$  
%In other words, prove that for every $\epsilon > 0$, there exists $R > 0$ such that if $|z| \ge R$, then 
%$$\left|\frac{p(z)}{z^n} - a_n \right| \le \epsilon.$$ 
