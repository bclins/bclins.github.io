...
\question Esophageal cancer can spread to the lymph nodes, and the larger the tumor is, the more likely it is to spread.  A logistic regression model for the odds that the cancer has spread based on a sample of 31 patients is given below. 
$$\log(\text{odds}) = -2.086 + 0.5117 (\text{size})$$
where the explanatory variable is the size of the tumor in centimeters. 
\begin{parts}
