...
\item[Discrete expected value \& variance]
$$E(X) = \mu = \sum p_k x_k$$
$$\Var(X) = \sigma^2 = \sum p_k (x_k - \mu)^2$$

\item[Expected value \& variance rules]
...
\Var(cX) &= c^2 \Var(X) 
\end{align*}
$^*$\textit{Only if $X$ and $Y$ are independent.}


\item[General form of a confidence interval] 
$$\text{estimate} \pm \text{margin of error}, \text{ where }$$
...
where $s = \sqrt{MSE}$ is the standard error of the residuals. Use $dF = n - 2$ for simple linear regression.
