...
\begin{solution}
Each step in the bisection method reduces the error by a factor of 2.  Initially the solution could be anywhere in the interval $[1,2]$, so our initial error could be as big as $\tfrac{1}{2}$ (since that is half the length of the interval).  After 10 steps, the worst case error would be 
...
\end{align*}
\begin{solution}
$$\mathbf{F}(\mathbf{x}) = \begin{bmatrix} x^2 - 4y^2 - 1 \\ x^2 + xy - 1 \end{bmatrix} ~~~~ \mathbf{J}(\mathbf{x}) = \begin{bmatrix} 2x & - 8y \\ 2x + y & x \end{bmatrix}.$$
\end{solution}
\vfill
