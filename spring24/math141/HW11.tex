...
\item In economics, if $C(x)$ is the cost to produce $x$ units of a commodity, then $\dfrac{C(x)}{x}$ is the \textbf{average cost} per unit produced.  The derivative $C'(x)$ is the \textbf{marginal cost} of each extra item. Use calculus to show that the average cost is minimized at a level of production $x$ where the average cost is equal to the marginal cost.   %Add the assumption that C''(x) > 0 everywhere so that an critical point is a local minimum. 
\vfill
...
% This one is also a little too hard... but maybe okay as a challenge problem? 
\begin{flushright}
